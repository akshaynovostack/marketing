/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { FunnelWhereUniqueInput } from "./FunnelWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { FunnelUpdateInput } from "./FunnelUpdateInput";

@ArgsType()
class UpdateFunnelArgs {
  @ApiProperty({
    required: true,
    type: () => FunnelWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => FunnelWhereUniqueInput)
  @Field(() => FunnelWhereUniqueInput, { nullable: false })
  where!: FunnelWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => FunnelUpdateInput,
  })
  @ValidateNested()
  @Type(() => FunnelUpdateInput)
  @Field(() => FunnelUpdateInput, { nullable: false })
  data!: FunnelUpdateInput;
}

export { UpdateFunnelArgs as UpdateFunnelArgs };
